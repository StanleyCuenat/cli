...
var nlbServiceAddCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		targetPort, err := cmd.Flags().GetInt64("target-port")
		if err != nil {
			return err
		}
		...
		healthcheckPort, err := cmd.Flags().GetInt64("healthcheck-port")
		if err != nil {
			return err
		}
		...
		if !gQuiet {
			cmd.Println("Service added successfully")
		}
...
func init() {
	...
	nlbServiceAddCmd.Flags().Int64("port", 0, "service port")
	nlbServiceAddCmd.Flags().Int64("target-port", 0, "port to forward traffic to on target instances")
	...
	nlbServiceAddCmd.Flags().Int64("healthcheck-port", 0, "service health checking port")
